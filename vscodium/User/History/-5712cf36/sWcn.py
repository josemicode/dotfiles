...
+            instance.apto = final
+        else:
+            # Si no, marcamos apto = True sólo cuando ninguna parte del texto
+            # contenga palabrotas. Cada item en retrieved_result tiene:
+            #    item['palabrotas']['contains'] == True|False
+            contains_any_swear = any(
+                entry.get('palabrotas', {}).get('contains', False)
+                for entry in results_list
+            )
+            # Si encontramos palabrota, apto = False; en caso contrario, True
